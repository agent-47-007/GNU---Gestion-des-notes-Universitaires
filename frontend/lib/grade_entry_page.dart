...
class _GradeEntryPageState extends State<GradeEntryPage> {
  ...
  GradeElement get element =>
      unit.elements.firstWhere((e) => e.id == elementId);
  ...
  Future<void> saveDraft() async {
    ...
    try {
      ...
      if (mounted)
        notice(
            'Enregistrement impossible. Les modifications restent dans la page.');
    } finally {
  ...
  void validate() {
    ...
    notice(
        'Saisie de ${element.code} vérifiée dans la démonstration. Aucune transmission effectuée.');
  }
  ...
  Future<void> simulateSubmission() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'La saisie de ${element.code} sera marquée comme transmise '
  ...
  Future<bool> confirmLeave() async {
    ...
    return await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text(
                'Enregistre le brouillon pour retrouver ces notes plus tard.'),
            actions: [
              TextButton(
                  onPressed: () => Navigator.pop(context, false),
  ...
  Widget build(BuildContext context) => PopScope(
        ...
        child: Scaffold(
          appBar: TeacherAppBar(
              section: TeacherSection.grades, beforeNavigate: confirmLeave),
          body: loading
                      ...
                      child: Column(
                        ...
                        children: [
                          panel(Column(
                              ...
                              children: [
                                const Text('DÉMONSTRATION · REGISTRE DE SAISIE',
                                    style: TextStyle(
                                        fontFamily: 'monospace', color: red)),
                                const SizedBox(height: 8),
                                Text('Saisie & Vérifications des Notes',
                                    style: Theme.of(context)
                                        .textTheme
                                        .headlineLarge),
                                const SizedBox(height: 8),
                                Text(
                                    '${book.students.length} étudiants de démonstration · '
                                    'Brouillons conservés dans ${store.locationLabel} · Transmission simulée'),
                                const SizedBox(height: 20),
                                selectors(),
                                const SizedBox(height: 16),
                                Text(
                                    'Registre ${unit.code} / ${element.code} · ${element.credits} crédits EC',
                                ...
                                Wrap(spacing: 12, runSpacing: 12, children: [
                                  for (final evaluation in element.evaluations)
                                    Chip(
                                        label: Text(
                                            '${evaluation.code} · ${evaluation.weight} %')),
                                ]),
                                const SizedBox(height: 10),
                                const Text(
                                    'Saisie sur 20, conversion sur 100. Le résultat de chaque EC est arrondi '
                                ...
                                const SizedBox(height: 8),
                                const Text(
                                    'Admission annuelle : chaque UE ≥ 35/100 (7/20), MGP ≥ 2/4 et aucun EL. '
                                    'EL correspond à une absence au rattrapage. Une note faible de CC, TP ou SN '
                                    'ne produit pas à elle seule une élimination.',
                          ...
                          panel(Column(
                              ...
                              children: [
                                Wrap(spacing: 8, runSpacing: 8, children: [
                                  ChoiceChip(
                                      label:
                                          const Text('Toutes les évaluations'),
                                      selected: evaluationId == null,
                                      onSelected: (_) =>
                                          setState(() => evaluationId = null)),
                                  for (final evaluation in element.evaluations)
                                    ChoiceChip(
                                        label: Text(evaluation.code),
                                        selected: evaluationId == evaluation.id,
                                        onSelected: (_) => setState(() =>
                                            evaluationId = evaluation.id)),
                                ]),
                                const SizedBox(height: 12),
                                const Text(
                                    'Entre une note entre 0 et 20, ou ABS pour une absence. '
                                    'Une cellule vide représente une note manquante. '
                                    'Une absence en session normale reste à régulariser avant finalisation.'),
                                const SizedBox(height: 8),
                                const Text(
                                    'Les résultats affichés sont des aperçus provisoires. '
                                    'Le grade concerne l’UE entière ; la décision annuelle dépend du bulletin complet.',
                                    style: TextStyle(color: slate)),
                              ])),
                          gradeTable(),
                          const SizedBox(height: 12),
                          ...
                          panel(Wrap(spacing: 12, runSpacing: 8, children: [
                            for (final label in [
                              'Importer Excel',
                              'Exporter la liste vierge',
                              'Exporter le bordereau'
                            ])
                              OutlinedButton.icon(
                                onPressed: () => notice(
                                    '$label : simulation. Aucun fichier n’a été importé ou créé.'),
  ...
  Widget selectors() => LayoutBuilder(builder: (context, constraints) {
        final ueSelector = DropdownButtonFormField<String>(
          ...
          decoration: const InputDecoration(
              labelText: 'Unité d’enseignement (UE)',
              border: OutlineInputBorder()),
          items: [
            for (final u in book.units)
              DropdownMenuItem(
                  value: u.id,
                  child: Text('${u.code} · ${u.title}',
        ...
        final ecSelector = DropdownButtonFormField<String>(
          ...
          isExpanded: true,
          decoration: const InputDecoration(
              labelText: 'Élément constitutif (EC)',
              border: OutlineInputBorder()),
          items: [
            for (final e in unit.elements)
              DropdownMenuItem(
                  value: e.id,
                  child: Text('${e.code} · ${e.title}',
                      overflow: TextOverflow.ellipsis))
          ],
          onChanged: (id) {
            if (id == null) return;
        ...
        if (constraints.maxWidth < 750)
          return Column(
              children: [ueSelector, const SizedBox(height: 16), ecSelector]);
        return Row(children: [
          Expanded(child: ueSelector),
          const SizedBox(width: 16),
          Expanded(child: ecSelector)
        ]);
      });
  ...
  Widget summaryPanel() {
    ...
    return panel(
      ...
      Text('Contrôle de ${element.code}',
          style: Theme.of(context).textTheme.titleLarge),
      const SizedBox(height: 12),
      Wrap(spacing: 12, runSpacing: 8, children: [
        metric(
            'Notes numériques', '${summary.numeric} / ${summary.total}', ink),
        metric('Manquantes', '${summary.missing}',
            summary.missing > 0 ? red : valid),
        metric('Absences', '${summary.absent}',
            summary.absent > 0 ? pending : valid),
        metric('Saisies invalides', '${summary.invalid}',
            summary.invalid > 0 ? red : valid),
        metric('Résultats UE < 35/100', '${summary.belowUeThreshold}',
            summary.belowUeThreshold > 0 ? red : valid),
      ]),
      const SizedBox(height: 12),
      Text(
          'Complétude numérique : ${summary.completionPercent.toStringAsFixed(1)} % · '
  ...
  Widget metric(String label, String value, Color color) => Container(
        ...
        decoration:
            BoxDecoration(color: paper, borderRadius: BorderRadius.circular(8)),
  ...
  Widget gradeTable() {
    final evaluations = element.evaluations
        .where((e) => evaluationId == null || e.id == evaluationId)
        .toList();
    return Card(
        ...
        child: DataTable(
          ...
          columns: [
            ...
            for (final e in evaluations)
              DataColumn(label: Text('${e.code} /20\n${e.weight} %')),
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          Text(student.matricule,
                              style: const TextStyle(
                                  fontFamily: 'monospace', color: slate)),
                        ]))),
                for (final e in evaluations) DataCell(noteField(student, e)),
                DataCell(Text(
                    formatResult(book.ecResult(elementId, student.id).raw100))),
                DataCell(Text(
                    '${book.ecResult(elementId, student.id).score100 ?? '—'}')),
                DataCell(Text(
                    '${book.ueResult(unitId, student.id).score100 ?? '—'}')),
                DataCell(Text(gradeText(student.id))),
                DataCell(SizedBox(
                    width: 230,
                    child: Text(resultStatus(student.id),
                        style: TextStyle(color: resultColor(student.id))))),
              ]),
          ],
        ),
      ),
  ...
  String resultStatus(String studentId) {
    if (element.evaluations.any((e) =>
        book.note(elementId, studentId, e.id).kind == GradeNoteKind.invalid))
      return 'Saisie invalide à corriger';
    if (element.evaluations.any((e) =>
        book.note(elementId, studentId, e.id).kind == GradeNoteKind.missing))
      return 'Note manquante';
    if (element.evaluations.any((e) =>
        book.note(elementId, studentId, e.id).kind == GradeNoteKind.absent &&
        !e.isResit)) return 'Absence à régulariser';
    final result = book.ueResult(unitId, studentId);
    if (result.eliminated) return 'EL · Absence au rattrapage';
    if (!result.complete) return 'Autre EC à compléter';
    if (result.belowAnnualThreshold)
      return 'UE sous le minimum annuel (35/100)';
    ...
    return SizedBox(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          errorText:
              note.kind == GradeNoteKind.invalid ? 'Note invalide' : null,
  ...
  Widget commitmentPanel() {
    ...
    return panel(
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Vérification & transmission de ${element.code}',
          style: Theme.of(context).textTheme.titleLarge),
      const SizedBox(height: 10),
      Text(statusLabel,
          key: const ValueKey('entry-status'),
          style: const TextStyle(fontWeight: FontWeight.bold)),
      const SizedBox(height: 8),
      Text(dirty
          ? 'Modifications non enregistrées.'
          : storageMessage ??
              'Les données de démonstration peuvent être modifiées.'),
      if (issues.isNotEmpty)
        Padding(
            padding: const EdgeInsets.only(top: 8),
            child: Text('Finalisation bloquée : ${issues.join(' ')}',
      ...
      Wrap(spacing: 12, runSpacing: 10, children: [
        OutlinedButton.icon(
            ...
            label:
                Text(saving ? 'Enregistrement…' : 'Enregistrer le brouillon')),
        FilledButton.icon(
            key: const ValueKey('validate-entry'),
            onPressed: status == GradeEntryStatus.draft && issues.isEmpty
                ? validate
                : null,
            icon: const Icon(Icons.fact_check_outlined),
            label: const Text('Vérifier & valider l’EC')),
        FilledButton.icon(
            key: const ValueKey('submit-entry'),
            onPressed: status == GradeEntryStatus.validated
                ? simulateSubmission
                : null,
            icon: const Icon(Icons.send_outlined),
            label: const Text('Simuler la transmission au Décanat')),
        if (submitted)
          OutlinedButton(
              onPressed: () => setState(() => book.reopenEc(elementId)),
              child: const Text('Reprendre la saisie')),
      ]),
      const SizedBox(height: 12),
      const Text(
          'Toute modification de note impose une nouvelle validation de l’EC. '
          'La publication officielle des résultats relève ensuite de l’agent habilité.',
          style: TextStyle(color: slate)),
    ]));
  }
}
