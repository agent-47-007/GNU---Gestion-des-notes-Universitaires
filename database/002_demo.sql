BEGIN;
SET LOCAL search_path TO gnu, public;
...
SELECT
    ...
    6
FROM ue
WHERE code_ue = 'INF331';

UPDATE ue
SET statut_catalogue = 'ACTIF'
WHERE code_ue = 'INF331';
...
-- Le plan doit être créé en BROUILLON : le trigger exige un total de 100 % avant activation.
INSERT INTO plan_evaluation(affectation_id,version_plan,statut_plan,portee)
SELECT id,1,'BROUILLON','CLASSE' FROM affectation;

-- Lier chaque plan à l'évaluation de la même affectation.
INSERT INTO ligne_plan(plan_id,evaluation_id,retenue,poids_evaluation)
SELECT p.id,e.id,true,100
FROM plan_evaluation p
JOIN evaluation e ON e.affectation_id=p.affectation_id;

-- Le total retenu vaut maintenant 100 %, le plan peut devenir ACTIF.
UPDATE plan_evaluation
SET statut_plan='ACTIF'
WHERE statut_plan='BROUILLON';
...
SELECT set_config('gnu.origine', 'SAISIE', false);
