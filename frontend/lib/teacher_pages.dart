...
class TeacherAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  const TeacherAppBar({super.key, required this.section, this.beforeNavigate});

  @override
  Size get preferredSize => const Size.fromHeight(72);

  Future<void> _go(BuildContext context, TeacherSection destination) async {
  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      ...
      elevation: 0,
      title: LayoutBuilder(builder: (context, constraints) => _TBrand(compact: constraints.maxWidth < 300)),
      actions: compact
          ? [
              ...
              _TProfileButton(compact: true, beforeNavigate: beforeNavigate),
              const SizedBox(width: 8),
            ]
          : [
              ...buttons,
              const SizedBox(width: 14),
              _TProfileButton(beforeNavigate: beforeNavigate),
...
class _TBrand extends StatelessWidget {
  final bool compact;
  const _TBrand({this.compact = false});
  ...
  Widget build(BuildContext context) => Row(
        ...
        children: [
          ...
          Column(
            ...
            children: [
              const Text('GNU', style: TextStyle(fontFamily: 'Fraunces', fontWeight: FontWeight.bold, fontSize: 22, color: ink)),
              if (!compact) const Text('SYSTÈME ACADÉMIQUE LMD · PORTAIL', style: TextStyle(fontFamily: 'monospace', letterSpacing: .65, fontSize: 8, color: slate)),
              if (!compact) const Text('OFFICIEL', style: TextStyle(fontFamily: 'monospace', letterSpacing: .65, fontSize: 8, color: slate)),
            ],
          ),
        ],
      );
...
class _TProfileButton extends StatelessWidget {
  final bool compact;
  final Future<bool> Function()? beforeNavigate;
  const _TProfileButton({this.compact = false, this.beforeNavigate});
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          final cards = [
            ...
            _TDashboardPillar(
              ...
              color: red,
              lines: const ['#REC-2024-0841 · Note TP omise (ABS)', '#REC-2024-0842 · Erreur note CC 06/20', '10 notes à vérifier · Exemple de suivi'],
...
class _TComplianceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(
        ...
        child: Padding(
          padding: const EdgeInsets.all(18),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('REPÈRES DU SYSTÈME LMD', style: TextStyle(fontFamily: 'monospace', fontSize: 9, color: slate)),
            const SizedBox(height: 10),
            const Text('Règles de calcul', style: TextStyle(fontFamily: 'Fraunces', fontSize: 19, fontWeight: FontWeight.bold, color: ink)),
            const SizedBox(height: 8),
            const Text('Aperçu de démonstration · Données illustratives.', style: TextStyle(fontSize: 11, color: slate)),
            const SizedBox(height: 13),
            const _TCheck(text: 'Exemple de plan EC : CC 20% · TP 30% · SN 50%'),
            const _TCheck(text: 'Admission annuelle : chaque UE arrondie ≥ 35/100'),
            const _TCheck(text: 'Admission annuelle : MGP ≥ 2/4, aucune UE EL'),
            const _TCheck(text: 'EL : absence au rattrapage uniquement', warning: true),
            const SizedBox(height: 13),
            SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Export de démonstration : aucun fichier créé.'))), icon: const Icon(Icons.download_outlined, size: 17), label: const Text('Simuler l’export de la fiche'))),
          ]),
        ),
      );
}
  ...
  Widget build(BuildContext context) => Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const Text('% EXEMPLE DE PLAN EC (INF301 · EC 301.1) :', style: TextStyle(fontFamily: 'monospace', fontSize: 9, color: slate)),
            ...
            const SizedBox(height: 7),
            const Text('▲ Admission annuelle : chaque UE arrondie ≥ 35/100 · EL = absence au rattrapage', style: TextStyle(fontFamily: 'monospace', color: red, fontSize: 9)),
            const SizedBox(height: 14),
            _TUnitBlock(
              code: 'INF301',
  ...
  Widget build(BuildContext context) => Card(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 6),
            const _TPlanInfo(label: 'EXEMPLE DE PLAN EC', value: '20% CC · 30% TP · 50% SN'),
