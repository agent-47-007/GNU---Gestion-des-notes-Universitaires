...
class GradeNote {
  ...
  factory GradeNote.parse(String raw) {
    ...
    return GradeNote._(
        raw, GradeNoteKind.numeric, _Fraction(numerator, denominator), null);
  }
...
class GradeEntryBook {
  ...
  GradeEntryBook({
    ...
    for (final unit in units) {
      if (unit.elements.isEmpty ||
          unit.elements.length > 2 ||
          unit.credits <= 0) {
        throw ArgumentError(
            'Une UE active contient 1 à 2 EC et des crédits positifs.');
      }
      for (final element in unit.elements) {
        if (_elements.containsKey(element.id) ||
          ...
          throw ArgumentError(
              'EC ou pondérations du plan d’évaluation invalides.');
        }
        ...
        _notes[element.id] = {
          for (final student in students)
            student.id: {
              for (final evaluation in element.evaluations) evaluation.id: ''
            },
        };
  ...
  void updateNote(
      String ecId, String studentId, String evaluationId, String raw) {
  ...
  GradeResult ueResult(String ueId, String studentId) {
    ...
    for (final element in unit.elements) {
      ...
      elementCredits += element.credits;
    }
    if (eliminated) return const GradeResult.eliminated();
    // UE credits weight the bulletin; EC credits weight the UE calculation.
    final result = _Fraction.integer(total).dividedBy(elementCredits);
    ...
    for (final student in students) {
      for (final evaluation in ec.evaluations) {
        switch (note(ecId, student.id, evaluation.id).kind) {
          case GradeNoteKind.numeric:
            numeric++;
          case GradeNoteKind.missing:
            missing++;
          case GradeNoteKind.absent:
            absent++;
          case GradeNoteKind.invalid:
            invalid++;
        }
      }
      if (ueResult(unitForElement(ecId).id, student.id).belowAnnualThreshold)
        below++;
    }
    return GradeSummary(
        total: students.length * ec.evaluations.length,
        numeric: numeric,
        missing: missing,
        absent: absent,
        invalid: invalid,
  ...
  GradeEntryStatus status(String ecId) =>
      _statuses[ecId] ??
      (throw ArgumentError.value(ecId, 'ecId', 'EC inconnu'));
  ...
  factory GradeEntryBook.fromDraft(String encoded) {
    if (encoded.length > 100000)
      throw const FormatException('Brouillon trop volumineux.');
    ...
    if (decoded is! Map<String, dynamic> ||
      ...
      throw const FormatException(
          'Brouillon incompatible avec ce catalogue et cette classe.');
    }
    final dynamic data = decoded['notes'];
    if (data is! Map<String, dynamic> ||
        !_hasKeys(data, book._notes.keys.toSet())) {
    ...
    for (final ec in book._elements.values) {
      ...
      if (rows is! Map<String, dynamic> ||
        ...
        throw const FormatException(
            'Liste des étudiants du brouillon invalide.');
      }
      for (final student in book.students) {
        ...
        if (cells is! Map<String, dynamic> ||
          ...
          throw const FormatException(
              'Plan d’évaluation du brouillon invalide.');
        }
        for (final evaluation in ec.evaluations) {
...
const _demoUnits = [
  GradeUnit(
      ...
      elements: [
        GradeElement(
            ...
            credits: 3,
            evaluations: _demoPlanWithTp),
        GradeElement(
            id: 'inf301-ec2',
            code: 'INF301-EC2',
            title: 'Structures de données',
            credits: 2,
            evaluations: _demoPlanWithoutTp),
      ]),
  GradeUnit(
      id: 'inf305',
      code: 'INF305',
  ...
  GradeUnit(
      id: 'inf201',
      code: 'INF201',
      title: 'C/C++ fondamental',
      credits: 4,
      elements: [
        GradeElement(
            id: 'inf201-ec1',
            code: 'INF201-EC1',
            title: 'Programmation C/C++',
            credits: 4,
            evaluations: _demoPlanWithTp),
      ]),
];
...
const _demoStudents = [
  GradeStudent(
      id: '21U2094',
      matricule: '21U2094',
      name: 'ABENA ESSOMBA Jean-Marc',
      group: 'G1'),
  GradeStudent(
      id: '21U2412',
      matricule: '21U2412',
      name: 'BIKOUÉ NDOUMBE Carine',
      group: 'G1'),
  GradeStudent(
      id: '21U2991',
      matricule: '21U2991',
      name: 'EKANE TCHINDA Rostand',
      group: 'G2'),
  GradeStudent(
      id: '21U3104',
      matricule: '21U3104',
      name: 'KAMDEM WABO Ulrich',
      group: 'G2'),
  GradeStudent(
      id: '21U2849',
      matricule: '21U2849',
      name: 'DJOUKA FOTSO Franck Kevin',
      group: 'G1'),
  GradeStudent(
      id: '21U2118',
      matricule: '21U2118',
      name: 'MANGA ATANGANA Boris Cyrille',
      group: 'G2'),
  GradeStudent(
      id: '21U2505',
      matricule: '21U2505',
      name: 'NGO MBOCK Madeleine Viviane',
      group: 'G1'),
];
